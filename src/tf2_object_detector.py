...
class tf2_Detector:
    def __init__(self, label_dict: dict, path_to_checkpoint: str,
                 threshold: float = 0.5):
    ...
    def detect_from_image(self, img):
        ...
        det_boxes = self.ExtractBBoxes(bboxes, bclasses, bscores, im_width,
                                       im_height)
            ...
            if bscores[idx] >= self.Threshold:
                ...
                if class_label == 'Tray':
                    ...
                    if tray_score < float(bscores[idx]):
                        tray_box = [x_min, y_min, x_max, y_max, class_label,
                                    float(bscores[idx])]
                        tray_score = float(bscores[idx])
                    continue
                bbox.append([x_min, y_min, x_max, y_max, class_label,
                             float(bscores[idx])])
    ...
    def DisplayDetections(self, image, boxes_list, det_time=None):
        ...
        if det_time != None:
            ...
            fps_txt = str(fps) + " FPS"
            cv2.putText(img, fps_txt, (25, 30), cv2.FONT_HERSHEY_SIMPLEX,
                        0.8, (0, 0, 0), 2)
